...
function Visualize() { 
  ...
  const width = window.parent.outerWidth/3;
  const height = window.parent.outerHeight/5;
  ...
  useEffect(()=>{
    fetch("https://backend-service-dot-ecohacks-361600.wl.r.appspot.com/getcountries", {
method: 'POST'
}).then(res=>{
    res.json().then(data=>{
        let countries = [];
  ...
  let buildCharts = selectedCountries.map(country=>{
    return fetch("https://backend-service-dot-ecohacks-361600.wl.r.appspot.com/getmeandata",{
      headers:{
...
let buildLineCharts = selectedCountries.map(country=>{
  return fetch("https://backend-service-dot-ecohacks-361600.wl.r.appspot.com/getalldata",{
    headers:{
      'Access-Control-Allow-Origin':'*',
      'Content-Type': 'application/json'
      },
...
useEffect(()=>{
  ...
  fetch("https://backend-service-dot-ecohacks-361600.wl.r.appspot.com/predictAQI",{
    method: 'POST',
    headers:{
      'Access-Control-Allow-Origin':'*',
      'Content-Type': 'application/json'
    ...
    body: JSON.stringify({
      "time": predictedDate.getFullYear()+'-'+(predictedDate.getMonth()+1)+'-'+predictedDate.getDate(),
      "code": country.value
    })
  }).then((res)=>{
    res.json().then(data=>{
